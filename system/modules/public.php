...
if($logged){
	...
	if($page_cnt){
		...
		$row = $db->super_query("SELECT id, group_real, title, descr, traf, ulist, photo, date, admin, feedback, comments, real_admin, rec_num, del, ban, adres, audio_num, forum_num, discussion, status_text, web, videos_num, cover, cover_pos FROM `".PREFIX."_communities` WHERE ".$sql_where."");
	...
	if($row['del'] == 1){
		...
		//Проверка группы
if($row['group_real'] == 1){
$tpl->set('{group_real}', '<img style="margin-left:5px" src="/templates/Default/images/icons/verifi.png" title="Подтверждённая группа">');
} else {
$tpl->set('{group_real}', '');
}
		...
		if($row['videos_num']){
			...
			foreach($sql_videos as $row_video){
				...
				$videos .= "
<div class=\"onevideo\" style=\"width:190px; display:inline-block\">
  <a href=\"/video{$row_video['owner_user_id']}_{$row_video['id']}\" onClick=\"videos.show({$row_video['id']}, this.href, '/{$row['adres']}'); return false\">
  <a  href=\"/video{$row_video['owner_user_id']}_{$row_video['id']}\" onClick=\"videos.show({id}, this.href); return false\">
  <div class=\"video_menu\">
    <div class=\"onevideo_title\">{$row_video['title']}</div>
  </div>
  </a>
  <div class=\"onevideo_img\"><img src=\"{$row_video['photo']}\" /></div>
  </a>
  <input type=\"hidden\" value=\"{id}\" id=\"onevideo\" />
		...
		if($row['photo']){
			...
			$tpl->set('{cover-param-3}', 'style="position:absolute;z-index:2;display:block;margin-left:497px"');
