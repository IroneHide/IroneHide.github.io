...
if($logged){
	...
	if($type == 1){ //Если критерий поиск "по людям"
		$sql_query = "SELECT user_id, user_real, user_search_pref, user_photo, user_birthday, user_country_city_name, user_last_visit, user_logged_mobile FROM `".PREFIX."_users` {$where_sql_gen} {$sql_sort} ORDER by `user_rating` DESC LIMIT {$limit_page}, {$gcount}";
		...
		$sql_query = "SELECT id, group_real, title, photo, traf, adres FROM `".PREFIX."_communities` WHERE title LIKE '%{$query}%' AND del = '0' AND ban = '0' ORDER by `traf` DESC, `photo` DESC LIMIT {$limit_page}, {$gcount}";
