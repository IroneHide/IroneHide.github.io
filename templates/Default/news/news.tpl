...
<div style="float:left;width:50px;padding:3px;"><div class="ava_mini"><a href="/{link}{author-id}" onClick="Page.Go(this.href); return false"><img src="{ava}" alt="" /></a></div></div>
<div style="position:absolute;margin-top:45px;margin-left:5px;">{online}</div>
<div class="wallauthor" ><a href="/{link}{author-id}" onClick="Page.Go(this.href); return false">{author}</a> <span class="color777">{action-type-updates}</span>
[wall]<div class="wall_tell_all cursor_pointer" onMouseOver="myhtml.title('{rec-id}', 'Отправить в сообщество или другу', 'wall_tell_all_')" onClick="Repost.Box('{rec-id}'[groups], 1[/groups]); return false "id="wall_tell_all_{rec-id}" style="margin-top:5px;margin-right:5px"></div><div class="wall_tell cursor_pointer wall_tell_fornews" style="margin-top:5px" onMouseOver="myhtml.title('{rec-id}', 'Рассказать друзьям', 'wall_tell_')" onClick="[wall-func]wall.tell[/wall-func][groups]groups.wall_tell[/groups]('{rec-id}'); return false" id="wall_tell_{rec-id}"></div>
 <div class="wall_tell_ok no_display wall_tell_fornews" id="wall_ok_tell_{rec-id}"></div>[/wall]
</div>
{date}
<div style="float:left;width:880px;margin-left:0px">

<div class="walltext" style="margin-top:3px">{comment}</div>
<div class="infowalltext_f clear">
<div class="fl_l" style="margin-left:10px;">{action-type}  [comments-link]<span id="fast_comm_link_{rec-id}" class="fast_comm_link">&nbsp;|&nbsp; <a href="/" id="fast_link_{rec-id}" onClick="wall.open_fast_form('{rec-id}'); wall.fast_open_textarea('{rec-id}'); return false">Комментировать</a></span>[/comments-link]</div>
[wall]<div class="public_likes_user_block no_display" id="public_likes_user_block{rec-id}" onMouseOver="groups.wall_like_users_five('{rec-id}'[wall-func], 'uPages'[/wall-func])" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" style="margin-left:645px">
   ...
   <div class="public_like_strelka" style="margin-left:145px;"></div>
  </div>
  ...
  <div class="fl_r public_wall_like cursor_pointer" onClick="{like-js-function}" onMouseOver="groups.wall_like_users_five('{rec-id}'[wall-func], 'uPages'[/wall-func])" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" id="wall_like_link{rec-id}">
   <div class="fl_l" id="wall_like_active"></div>
      <b id="wall_like_cnt{rec-id}" style="float:right;margin-top:1px;font-size:13px;" class="{yes-like-color}">{likes}</b>
...
<div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
 <textarea class="wall_inpst fast_form_width wall_fast_text" style="height:33px;color:#000;margin:0px;;width:828px" id="fast_text_{rec-id}"
...
[comment-form]<div class="wall_fast_opened_form" id="fast_form">
 <input type="text" class="wall_inpst fast_form_width wall_fast_input" value="Комментировать..." id="fast_inpt_{rec-id}" onMouseDown="wall.fast_open_textarea('{rec-id}', 2); return false" style="margin:0px;width:858px" />
 <div class="no_display wall_fast_texatrea" id="fast_textarea_{rec-id}">
 <textarea class="wall_inpst fast_form_width wall_fast_text" style="height:33px;color:#000;margin:0px;;width:828px" id="fast_text_{rec-id}"
	onKeyPress="if(event.keyCode == 10 || (event.ctrlKey && event.keyCode == 13))[wall-func]wall.fast_send[/wall-func][groups]groups.wall_send_comm[/groups]('{rec-id}', '{author-id}', 1)"
 ></textarea>
