...
var fave = {
	add: function(fave_id){
		...
		$.post('/index.php?go=fave&act=add', {fave_id: fave_id}, function(data){
			...
			$('#text_add_fave').html('<span class="tooltipe"><i class="icon-bookmark"></i><span class="classic">Добавить в закладки</span></span>');
			$('#addfave_load').hide();
		});
	},
	delet: function(fave_id){
		...
		$.post('/index.php?go=fave&act=delet', {fave_id: fave_id}, function(data){
			...
			$('#text_add_fave').html('<span class="tooltipe"><i class="icon-bookmark-empty"></i><span class="classic">Убрать из закладок</span></span>');
			$('#addfave_load').hide();
		});
	},
...
var subscriptions = {
	add: function(for_user_id){
		...
		$.post('/index.php?go=subscriptions&act=add', {for_user_id: for_user_id}, function(d){
			...
			$('#text_add_subscript').html('<span class="tooltipe"><i class="icon-loop-outline"></i><span class="classic">Подписаться</span></span>');
	...
	del: function(del_user_id){
		...
		$.post('/index.php?go=subscriptions&act=del', {del_user_id: del_user_id}, function(){
			...
			$('#text_add_subscript').html('<span class="tooltipe"><i class="icon-loop-alt-outline"></i><span class="classic">Отписаться</span></span>');
...
var settings = {
	...
	addblacklist: function(bad_user_id){
		...
		$.post('/index.php?go=settings&act=addblacklist', {bad_user_id: bad_user_id}, function(){
			...
			$('#text_add_blacklist').html('<span class="tooltipee"><i class="icon-lock"></i><span class="classic" style="left:-50px;">Заблокировать</span></span>');
	...
	delblacklist: function(bad_user_id, type){
		...
		$.post('/index.php?go=settings&act=delblacklist', {bad_user_id: bad_user_id}, function(){
			if(type){
				...
				$('#text_add_blacklist').html('<span class="tooltipee"><i class="icon-lock-open"></i><span class="classic">Разблокировать</span></span>');
				$('#addblacklist_load').hide();
			} else {
...
'<div class="photo_close" onClick="Photo.Close(\'\'); return false;"></div>'+
 '<div class="photo_bg" style="min-height:400px;margin-top:3%;">'+
	...
	read: function(msg_id, auth_id, my_id){
		if(auth_id != my_id && imrearstart){
			...
			$.post('/index.php?go=im&act=read', {msg_id: msg_id}, function(){
				imrearstart = 1;
				if(msg_num > 0)
					$('#new_msg').html("<div class=\"headm_newac\" style=\"margin-left:15px;margin-top:-10px;\">"+msg_num+"</div>");
				else
					$('#new_msg').html('');
...
var cover = {
	init: function(i, hi){
		...
		$("#les10_ex2").draggable({
			...
			drag: function(event, ui){
				...
				if(d >= 0){
					$("#les10_ex2").remove();
					$('#cover_restart').html('<div style="width:910px;height:'+hi+'px;position:relative;top:0px;z-index:1" id="les10_ex2"><img src="'+i+'" width="910" id="cover_img" /></div>');
					$('.cover_addut, .cover_descring').show();
					cover.init(i, hi);
				}
				h = parseInt('-'+(hi-230));
				if(d <= h){
					$("#les10_ex2").remove();
					$('#cover_restart').html('<div style="width:910px;height:'+hi+'px;position:relative;top:'+h+'px;z-index:1" id="les10_ex2"><img src="'+i+'" width="910" id="cover_img" /></div>');
					$('.cover_addut, .cover_descring').show();
					cover.init(i, hi);
				}
			}
	...
	cancel: function(t){
		...
		$('.cover_newpos').css('margin-left', '497px');
	...
	startedit: function(i, h, public_id){
		...
		$('.cover_newpos').css('margin-left', '300px');
