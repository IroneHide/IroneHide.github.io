<div id="photo_view_{id}" class="photo_view" onClick="Photo.setEvent(event, '{close-link}')">
  ...
  <div class="photo_rightcol">
    <div class="alb_name">{author}
      <a href="/albums/view/{aid}" class="fl_r" style="font-weight:normal; font-size:12px;" onClick="Page.Go(this.href); return false">Закрыть</a>
    </div>
    ...
    <div class="menuleft1" style="width:209px;"> [owner]
    ...
    <div id="pinfo_{id}" class="pinfo">
      <div class="photo_leftcol">
        ...
        <div class="photo_all_comm_bg" id="all_lnk_comm_{id}" style="width:196px;">Комментарии {comm_num}</div>
        </a>
        ...
        <textarea id="textcom_{id}" class="inpst fl_l" style="width:200px;color:#666;height:30px;-moz-border-radius: 2px ;-webkit-border-radius: 2px ;border-radius: 2px;-moz-background-clip: padding;-webkit-background-clip: padding-box;background-clip: padding-box;"></textarea>
        <div class="button_div fl_l" style="margin-top:5px;">
          <button id="add_comm" style="width:210px;" onClick="comments.add({id}); return false">Отправить</button>
        </div>
        [/add-comm] </div>
    </div>
  </div>
  <div class="clear"></div>
  <div id="save_crop_text{id}" class="save_crop_text no_display" style="padding:10px; width:760px; margin-top:0px;"> Укажите область, которая будет сохранена как фотография Вашей страницы
