[record]<div class="wallrecord" id="wall_record_{rec-id}">
 <div class="wan">
 
 <div class="ava_mini" id="ava_rec_{rec-id}"><a href="/u{user-id}" onClick="Page.Go(this.href); return false"><img src="{ava}" style="wallimg" alt="" title="" /></a>{online}</div>
 ...
 <div class="wallauthor" style="padding-left:0px"><a href="/u{user-id}" onClick="Page.Go(this.href); return false">{name}</a> <span class="color777">{type}</span></div>
 <a href="/wall{author-id}_{rec-id}" onClick="Page.Go(this.href); return false" class="online">{date}</a>
 </div>
 <div class="infowalltext_f wall_rec_autoresize">

 <div class="walltext">{text}</div>
 <div class="infowalltex clear">
  ...
  <div class="fl_r public_wall_like cursor_pointer" onClick="{like-js-function}" onMouseOver="groups.wall_like_users_five('{rec-id}', 'uPages')" onMouseOut="groups.wall_like_users_five_hide('{rec-id}')" id="wall_like_link{rec-id}">
      <b id="wall_like_cnt{rec-id}" class="public_wall_like_yes_color {yes-like-color}">{likes}</b>
